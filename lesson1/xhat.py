...
# 모듈의 사용
# 모듈이란? 자주 사용하는 코드, 유용한 코드를 논리적으로 묶어서 관리하고 사용할 수 있도록 하는 것
# 보통 하나의 파이썬 .py 파일이 하나의 모듈이 된다
# 모듈은 함수, 클래스, 변수, 실행 코드 등을 포함한다

# <모듈을 불러오는 형태>
# import 모듈명 as 별명 - 이런 형태로 불러오는 것도 가능하다
# from 모듈 import 함수 (as 별명) ---모듈 명을 생략함
# from 모듈 import * : 모듈에 있는 모든 내용을 불러온다



# GPIO: General Purpose Input Output. 범용 입력출력. 특정 포트에 대해 입력인지 출력인지 설정할 수 있음
...
# 모터 1의 속도를 조절한다. speed는 마이너스도 가능. 실행 중 ctrl + c를 누르면 모터가 정지된다
